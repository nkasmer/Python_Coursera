...
class Sprite:
    ...
    def draw(self, canvas):
        ...
        if self.animated == True:
            self.image_center = [self.image_center[0] + self.age * self.image_size[0], 
                                  self.image_center[1]]
    ...
    def collide(self, other_obj):
        obj_pos = other_obj.get_position()
        obj_radius = other_obj.get_radius()
        if dist(self.pos, obj_pos) < self.radius + obj_radius:
            return True
        else:
            return False
  
# group-object collide helper function
def group_collide(group, other_obj):
    for sprite in set(group):
        if sprite.collide(other_obj):
            group.remove(sprite)
            new_explosion = Sprite(other_obj.pos, [0, 0], 0, 0, 
...
def draw(canvas):
    ...
    # draw and update group of rocks
    process_sprite_group(rock_group, canvas)
    
    # draw and update group of missiles
    process_sprite_group(missile_group, canvas)
    
    # draw and update group of explosions
    process_sprite_group(explosion_group, canvas)
    
    # check out for collisions between the rocks and the ship
    if group_collide(rock_group, my_ship):
        lives -= 1
    
    # check out for collisions between the rocks and the ship
    missile_collisions = group_group_collide(missile_group, rock_group) 
    if missile_collisions > 0:
        score += missile_collisions * 10
...
# timer handler that spawns a rock_group    
def rock_spawner():
    global rock_group, score
    rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
    rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
    rock_vel[0] = rock_vel[0] * (.2 + score / 50)
    rock_vel[1] = rock_vel[1] * (.2 + score / 50)
    rock_avel = random.random() * .2 - .1
    new_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
    rock_pos = new_rock.get_position()
    ship_pos = my_ship.get_position()
    rock_rad = new_rock.get_radius()
    ship_rad = my_ship.get_radius()
    if dist(rock_pos, ship_pos) < rock_rad + ship_rad + 80:
        new_rock = None
    else:
